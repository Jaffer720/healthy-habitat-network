...
export default function Profile() {
  const { user } = useAuth(); // Your current logged-in user
  ...
  return (
    <div className="w-full max-w-3xl mx-auto bg-white rounded-2xl flex flex-col gap-6 p-8 shadow-md mt-6">
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className="block text-gray-500 text-sm">Age Group</label>
          <p className="text-lg font-semibold">{user.age_group || "Not specified"}</p>
        </div>
        ...
        <div>
          <label className="block text-gray-500 text-sm">Areas of Interest</label>
          <p className="text-lg font-semibold">
            {Array.isArray(user.areas_of_interest)
              ? user.areas_of_interest.join(", ")
              : user.areas_of_interest || "Not specified"}
          </p>
        </div>
      </div>
    </div>
